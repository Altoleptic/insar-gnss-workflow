...
def plot_global_velocity_map(before_df, after_df, stations_df, parameters_file, output_dir, title="Regional Velocity Map", prefix=""):
    """
    ...
    Args:
        ...
        prefix: String prefix for output filename (default: "")
    """
    ...
    # Calculate mean latitude for accurate aspect ratio adjustment
    mean_lat = (before_points["latitude"].min() + before_points["latitude"].max()) / 2
    
    # Calculate the appropriate aspect ratio based on the cosine of the mean latitude
    aspect_ratio = 1/np.cos(np.deg2rad(mean_lat))
    
    # Create figure with three vertically stacked subplots with improved proportions
    # Adjust figsize for better display with the aspect ratio correction
    fig_width = 14
    fig_height = 16  # Slightly reduced height for better proportion
    fig, axes = plt.subplots(3, 1, figsize=(fig_width, fig_height), sharex=True)
    
    # Subplot 1: Before alignment
    scatter1 = axes[0].scatter(before_points["longitude"], before_points["latitude"],
                              c=before_velocities, cmap="seismic", s=1, alpha=0.7, marker=".")
    axes[0].set_title("Before Alignment", fontsize=14, pad=5)
    axes[0].set_ylabel("Latitude (decimal degrees)", fontsize=12)
    axes[0].grid(alpha=0.5)
    # Set correct geographic aspect ratio to avoid map distortion
    axes[0].set_aspect(aspect_ratio)
    
    # Subplot 2: After alignment
    scatter2 = axes[1].scatter(after_points["longitude"], after_points["latitude"],
                              c=after_velocities, cmap="seismic", s=1, alpha=0.7, marker=".")
    axes[1].set_title("After Alignment", fontsize=14, pad=5)
    axes[1].set_ylabel("Latitude (decimal degrees)", fontsize=12)
    axes[1].grid(alpha=0.5)
    # Set correct geographic aspect ratio to avoid map distortion
    ...
    axes[2].set_title("Velocity Correction Plane", fontsize=14, pad=5)
    # Position the x-label with appropriate spacing
    # A smaller labelpad value moves the label closer to the axis (higher up, away from colorbar)
    axes[2].set_xlabel("Longitude (decimal degrees)", fontsize=11, labelpad=8)
    axes[2].set_ylabel("Latitude (decimal degrees)", fontsize=11)
    axes[2].grid(alpha=0.5)
    # Set correct geographic aspect ratio to avoid map distortion
    axes[2].set_aspect(aspect_ratio)
    
    # Add colorbars below each subplot with appropriate padding
    cbar1 = plt.colorbar(scatter1, ax=axes[0], orientation='horizontal', pad=0.05, shrink=0.8)
    cbar1.set_label("Velocity (mm/year)", fontsize=10)
    cbar1.ax.tick_params(labelsize=9)
    
    cbar2 = plt.colorbar(scatter2, ax=axes[1], orientation='horizontal', pad=0.05, shrink=0.8)
    cbar2.set_label("Velocity (mm/year)", fontsize=10)
    cbar2.ax.tick_params(labelsize=9)
    
    # Optimized padding for the third subplot's colorbar to avoid x-axis label overlap
    # while maintaining a compact layout
    cbar3 = plt.colorbar(scatter3, ax=axes[2], orientation='horizontal', pad=0.13, shrink=0.8)
    cbar3.set_label("Correction Value (mm/year)", fontsize=10)
    cbar3.ax.tick_params(labelsize=9)
    
    # Add GNSS station markers and labels to all subplots with improved visibility
    for _, station in stations_df.iterrows():
        station_name = station["Station"]
        station_lat = station["latitude"]
        station_lon = station["longitude"]
        for ax in axes:
            # Use triangle marker with white edge for better visibility
            ax.scatter(station_lon, station_lat, color="black", edgecolor="white", 
                      s=60, marker="^", zorder=10, linewidth=1.0)
            # Add station name with white outline for visibility against any background
            ax.text(station_lon + 0.002, station_lat + 0.002, station_name,
                    color="black", fontsize=9, ha="left", va="bottom", weight="bold",
    ...
    # Adjust subplot spacing - increase bottom margin to prevent x-axis label overlap with colorbar
    plt.subplots_adjust(left=0.09, right=0.95, bottom=0.09, top=0.95, hspace=0.15)
    
    # Save figure with descriptive filename
    output_path = os.path.join(output_dir, f"{prefix}_velocity_map_with_correction.png")
    # Use bbox_inches='tight' to automatically adjust borders and prevent any remaining overlaps
...
if __name__ == "__main__":
    ...
    plot_global_velocity_map(before_df, after_df, stations_df, parameters_file, plots_dir,
                         title="Regional Velocity Map", prefix="combined")
